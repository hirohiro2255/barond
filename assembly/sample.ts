...
function getRandomU64Number() {
  ...
  return BigInt.asUintN(
    64,
    BigInt(u1 | (u2 << 16n) | (u3 << 32n) | (u4 << 48n))
  );
}
...
function generateMagicNumber() {
  return BigInt.asUintN(
    64,
    getRandomU64Number() & getRandomU64Number() & getRandomU64Number()
  );
}
...
class Engine {
  ...
  private readonly pawnAttacks: bigint[][] = Array.from(
    Array(2),
    () => new Array(64)
  );
  ...
  private readonly bishopAttacks: bigint[][] = Array.from(
    Array(64),
    () => new Array(512)
  );
  // prettier-ignore
  ...
  private readonly rookAttacks: bigint[][] = Array.from(
    Array(64),
    () => new Array(4096)
  );
  // prettier-ignore
  ...
  PrintBoard(unicode = false) {
    ...
    console.log(
      `En passant: ${
        this.enpassant !== Square.no_sq ? SquareToCoords[this.enpassant] : 'no'
      }`
    );
    console.log(
      `Castling rights: ${this.castle & CastleRights.wk ? 'K' : '-'}${
        this.castle & CastleRights.wq ? 'Q' : '-'
      }${this.castle & CastleRights.bk ? 'k' : '-'}${
        this.castle & CastleRights.bq ? 'q' : '-'
      }`
    );
  ...
  private PrintMove(move: number) {
    console.log(
      `${SquareToCoords[this.GetMoveSource(move)]}${
        SquareToCoords[this.GetMoveTarget(move)]
      }${
        this.GetMovePromoted(move)
          ? this.promotedPieces[this.GetMovePromoted(move)]
          : ''
      }`
    );
  }
  ...
  private PrintMoveList(moves: number[]) {
    ...
    for (let moveCount = 0; moveCount < moves.length; moveCount++) {
      ...
      moveList.push({
        Move: `${SquareToCoords[this.GetMoveSource(move)]}${
          SquareToCoords[this.GetMoveTarget(move)]
        }${
          this.GetMovePromoted(move)
            ? this.promotedPieces[this.GetMovePromoted(move)]
  ...
  private PrintMoveScores(moves: number[]) {
    ...
    for (let i = 0; i < moves.length; i++) {
      scoreList.push({
        move: `${SquareToCoords[this.GetMoveSource(moves[i])]}${
          SquareToCoords[this.GetMoveTarget(moves[i])]
        }${
          this.GetMovePromoted(moves[i])
            ? this.promotedPieces[this.GetMovePromoted(moves[i])]
            : ' '
        }`,
  ...
  ParseFEN(fen: string) {
    ...
    for (let rank = 0; rank < 8; rank++) {
      ...
      for (const piece of piecesRank) {
        ...
        // Match piece
        if (
          piece.toLocaleLowerCase() >= 'a' &&
          piece.toLocaleLowerCase() <= 'z'
        ) {
    ...
    for (const castle of castling) {
      switch (castle) {
        case 'K':
          this.castle |= CastleRights.wk;
          break;
        case 'Q':
          this.castle |= CastleRights.wq;
          break;
        case 'k':
          this.castle |= CastleRights.bk;
          break;
        case 'q':
          this.castle |= CastleRights.bq;
          break;
      }
    }
    ...
    this.occupancies[SideToMove.Both] |=
      this.occupancies[SideToMove.White] | this.occupancies[SideToMove.Black];
  ...
  private MaskBishopAttacks(square: Square) {
    ...
    for (
      let r = targetRank + 1, f = targetFile + 1;
      r <= 6 && f <= 6;
      r++, f++
    ) {
  ...
  private GetQueenAttacks(square: Square, occupancy: bigint) {
    return (
      this.GetBishopAttacks(square, occupancy) |
  ...
  private InitSliderAttacks(bishop: Piece) {
    for (let square = 0; square < 64; square++) {
      ...
      for (let index = 0; index < occupancyIndicies; index++) {
        if (bishop) {
          const occupancy = this.SetOccupancy(
            index,
            relevantBitsCount,
            attackMask
          );
          ...
          const magicIndex =
            BigInt.asUintN(64, occupancy * this.bishopMagicNumbers[square]) >>
            (64n - this.bishopRelevantBits[square]);

          this.bishopAttacks[square][Number(magicIndex)] =
            this.GenerateBishopAttacksFly(square, occupancy);
        } else {
          const occupancy = this.SetOccupancy(
            index,
            relevantBitsCount,
          ...
          const magicIndex =
            BigInt.asUintN(64, occupancy * this.rookMagicNumbers[square]) >>
            (64n - this.rookRelevantBits[square]);

          this.rookAttacks[square][Number(magicIndex)] =
            this.GenerateRookAttacksFly(square, occupancy);
        }
      }
    }
  }
  ...
  private InitJumperAttacks() {
    for (let square = 0; square < 64; square++) {
      this.pawnAttacks[SideToMove.White][square] = this.MaskPawnAttacks(
        SideToMove.White,
        square
      );
      this.pawnAttacks[SideToMove.Black][square] = this.MaskPawnAttacks(
        SideToMove.Black,
        square
      );
  ...
  private IsSquareAttacked(square: Square, side: SideToMove) {
    ...
    if (this.pawnAttacks[side ^ 1][square] & pawnBoard) return 1;

    if (this.knightAttacks[square] & knightBoard) return 1;

    if (
      this.GetBishopAttacks(square, this.occupancies[SideToMove.Both]) &
      bishopsQueens
    )
      return 1;

    if (
      this.GetRookAttacks(square, this.occupancies[SideToMove.Both]) &
      rooksQueens
    )
      return 1;

    if (this.kingAttacks[square] & kingBoard) return 1;
  ...
  private FindMagicNumber(
    ...
    for (let count = 0; count < 100000000; count++) {
      ...
      if (
        this.CountBits((attackMask * magicNumber) & 0xff00000000000000n) < 6
      ) {
        continue;
      }
      ...
      for (let index = 0, fail = 0; !fail && index < n; index++) {
        const magicIndex = Number(
          BigInt.asIntN(
            16,
            (occupancies[index] * magicNumber) >> (64n - relevant_bits)
          )
        );
  ...
  private InitMagicNumbers() {
    for (let square = 0; square < 64; square++) {
      const decimalMN = this.FindMagicNumber(
        square,
        this.bishopRelevantBits[square],
        Piece.bishop
      );
  ...
  private GeneratePawnMoves(
    ...
    const ableToPush = (bitboard: bigint, empty: bigint) =>
      (side === SideToMove.White ? empty << 8n : empty >> 8n) & bitboard;
    const ableToDoublePush = (bitboard: bigint, empty: bigint) => {
      const targetRank =
        side === SideToMove.White ? 0x000000ff00000000n : 0x00000000ff000000n;
      const emptyRank =
        (side === SideToMove.White
          ? (empty & targetRank) << 8n
          : (empty & targetRank) >> 8n) & empty;
    ...
    while (bitboard) {
      ...
      const toSquare =
        side === SideToMove.White ? fromSquare - 8 : fromSquare + 8;
      ...
      if (!capturesOnly) {
          ...
          if (
            ...
            this.AddMove(
              ...
              this.EncodeMove(
                ...
                toSquare,
                piece,
                side === SideToMove.White ? Pieces.Q : Pieces.q,
                0,
                0,
                0,
                0
            ...
            this.AddMove(
              ...
              this.EncodeMove(
                ...
                toSquare,
                piece,
                side === SideToMove.White ? Pieces.R : Pieces.r,
                0,
                0,
                0,
                0
            ...
            this.AddMove(
              ...
              this.EncodeMove(
                ...
                toSquare,
                piece,
                side === SideToMove.White ? Pieces.B : Pieces.b,
                0,
                0,
                0,
                0
            ...
            this.AddMove(
              ...
              this.EncodeMove(
                ...
                toSquare,
                piece,
                side === SideToMove.White ? Pieces.N : Pieces.n,
                0,
                0,
                0,
                0
            ...
            this.AddMove(
              moves,
              this.EncodeMove(fromSquare, toSquare, piece, 0, 0, 0, 0, 0)
            );
          }
        }
        ...
        if (this.GetBit(bitboard, fromSquare) & pDoublePush) {
          this.AddMove(
            ...
            this.EncodeMove(
              fromSquare,
              side === SideToMove.White ? fromSquare - 16 : fromSquare + 16,
              piece,
              0,
              0,
              1,
      ...
      // attacks
      attacks =
        this.pawnAttacks[side][fromSquare] &
        this.occupancies[
          side === SideToMove.White ? SideToMove.Black : SideToMove.White
        ];
      ...
      while (attacks) {
        ...
        if (
          ...
          this.AddMove(
            moves,
            this.EncodeMove(fromSquare, targetSquare, piece, 0, 1, 0, 0, 0)
          );
        }
      ...
      if (this.enpassant !== Square.no_sq) {
        const enpassantAttacks =
          this.pawnAttacks[this.side][fromSquare] &
          (1n << squareBigInt[this.enpassant]);

        if (enpassantAttacks) {
          const target = this.GetLS1B(enpassantAttacks);
          this.AddMove(
            moves,
            this.EncodeMove(fromSquare, target, piece, 0, 1, 0, 1, 0)
          );
        }
      }
  ...
  private GenerateMoves(moves: number[], capturesOnly = false) {
    ...
    this.GeneratePawnMoves(
      this.side,
      moves,
      this.side === SideToMove.White ? wpawnBB : bpawnBB,
      capturesOnly
    );
    ...
    for (let piece = Pieces.P; piece <= Pieces.k; piece++) {
        ...
        if (this.side === SideToMove.White) {
          if (piece === Pieces.K) {
            ...
            if (this.castle & CastleRights.wk) {
              ...
              if (
                !this.GetBit(this.occupancies[SideToMove.Both], Square.f1) &&
                !this.GetBit(this.occupancies[SideToMove.Both], Square.g1)
              ) {
                // Make sure king and adjacent squares are not attacked
                if (
                  !this.IsSquareAttacked(Square.e1, SideToMove.Black) &&
                  !this.IsSquareAttacked(Square.f1, SideToMove.Black)
                ) {
                  this.AddMove(
                    moves,
                    this.EncodeMove(Square.e1, Square.g1, piece, 0, 0, 0, 0, 1)
                  );
                }
              }
            }
            ...
            if (this.castle & CastleRights.wq) {
              if (
                !this.GetBit(this.occupancies[SideToMove.Both], Square.d1) &&
                !this.GetBit(this.occupancies[SideToMove.Both], Square.c1) &&
                !this.GetBit(this.occupancies[SideToMove.Both], Square.b1)
              ) {
                // Make sure king and adjacent squares are not attacked
                if (
                  !this.IsSquareAttacked(Square.e1, SideToMove.Black) &&
                  !this.IsSquareAttacked(Square.d1, SideToMove.Black)
                ) {
                  this.AddMove(
                    moves,
                    this.EncodeMove(Square.e1, Square.c1, piece, 0, 0, 0, 0, 1)
                  );
                }
              }
            }
        ...
        // black castling moves
          ...
          if (piece === Pieces.k) {
            ...
            if (this.castle & CastleRights.bk) {
              // Make sure squares between king and rook are empty
              if (
                !this.GetBit(this.occupancies[SideToMove.Both], Square.f8) &&
                !this.GetBit(this.occupancies[SideToMove.Both], Square.g8)
              ) {
                // Make sure king and adjacent squares are not attacked
                if (
                  !this.IsSquareAttacked(Square.e8, SideToMove.White) &&
                  !this.IsSquareAttacked(Square.f8, SideToMove.White)
                ) {
                  this.AddMove(
                    moves,
                    this.EncodeMove(Square.e8, Square.g8, piece, 0, 0, 0, 0, 1)
                  );
                }
              }
            }
            ...
            if (this.castle & CastleRights.bq) {
              if (
                !this.GetBit(this.occupancies[SideToMove.Both], Square.d8) &&
                !this.GetBit(this.occupancies[SideToMove.Both], Square.c8) &&
                !this.GetBit(this.occupancies[SideToMove.Both], Square.b8)
              ) {
                // Make sure king and adjacent squares are not attacked
                if (
                  !this.IsSquareAttacked(Square.e8, SideToMove.White) &&
                  !this.IsSquareAttacked(Square.d8, SideToMove.White)
                ) {
                  this.AddMove(
                    moves,
                    this.EncodeMove(Square.e8, Square.c8, piece, 0, 0, 0, 0, 1)
                  );
                }
              }
            }
      ...
      if (
        this.side === SideToMove.White ? piece === Pieces.N : piece === Pieces.n
      ) {
        while (bitboard) {
          const fromSquare = this.GetLS1B(bitboard);

          let attacks =
            this.knightAttacks[fromSquare] & ~this.occupancies[this.side];

          while (attacks) {
            const toSquare = this.GetLS1B(attacks);

            const isCapture = this.GetBit(
              this.side === SideToMove.White
                ? this.occupancies[SideToMove.Black]
                : this.occupancies[SideToMove.White],
              toSquare
            );
            ...
            if (!capturesOnly && !isCapture) {
              ...
              this.AddMove(
                moves,
                this.EncodeMove(fromSquare, toSquare, piece, 0, 1, 0, 0, 0)
              );
            }
      ...
      if (
        this.side === SideToMove.White ? piece === Pieces.B : piece === Pieces.b
      ) {
        while (bitboard) {
          const fromSquare = this.GetLS1B(bitboard);
      ...
      // generate rook moves
      if (
        this.side === SideToMove.White ? piece === Pieces.R : piece === Pieces.r
      ) {
        while (bitboard) {
          const fromSquare = this.GetLS1B(bitboard);

          let attacks =
            this.GetRookAttacks(fromSquare, this.occupancies[SideToMove.Both]) &
            ~this.occupancies[this.side];
      ...
      if (
        this.side === SideToMove.White ? piece === Pieces.Q : piece === Pieces.q
      ) {
        while (bitboard) {
          const fromSquare = this.GetLS1B(bitboard);

          let attacks =
            this.GetQueenAttacks(
              fromSquare,
              this.occupancies[SideToMove.Both]
            ) & ~this.occupancies[this.side];
      ...
      // generate king moves
      if (
        this.side === SideToMove.White ? piece === Pieces.K : piece === Pieces.k
      ) {
        while (bitboard) {
          const fromSquare = this.GetLS1B(bitboard);

          let attacks =
            this.kingAttacks[fromSquare] & ~this.occupancies[this.side];
  ...
  private GetMoveSource(move: number) {
    return move & 0x3f;
  }
  private GetMoveTarget(move: number) {
    return (move & 0xfc0) >> 6;
  }
  private GetMovePiece(move: number) {
    return (move & 0xf000) >> 12;
  }
  private GetMovePromoted(move: number) {
    return (move & 0xf0000) >> 16;
  }
  private GetMoveCapture(move: number) {
    return move & 0x100000;
  }
  private GetMoveDouble(move: number) {
    return move & 0x200000;
  }
  private GetMoveEnPassant(move: number) {
    return move & 0x400000;
  }
  private GetMoveCastling(move: number) {
  ...
  private MakeMove(move: number) {
    ...
    if (promoted) {
      if (this.side === SideToMove.White) {
        this.bitboards[piece] = this.RemoveBit(
          this.bitboards[Pieces.P],
          toSquare
        );
      ...
      this.bitboards[promoted] = this.SetBit(
        this.bitboards[promoted],
        toSquare
      );
    ...
    if (enpassant) {
      ...
      if (this.side === SideToMove.White) {
        this.bitboards[Pieces.p] = this.RemoveBit(
          this.bitboards[Pieces.p],
          toSquare + 8
        );
    ...
    if (castling) {
      switch (toSquare) {
        // white king side
        case Square.g1: {
          this.bitboards[Pieces.R] = this.RemoveBit(
            this.bitboards[Pieces.R],
            Square.h1
          );
          this.bitboards[Pieces.R] = this.SetBit(
            this.bitboards[Pieces.R],
            Square.f1
          );
    ...
    this.occupancies[SideToMove.White] =
      this.bitboards[Pieces.P] |
      this.bitboards[Pieces.N] |
      this.bitboards[Pieces.B] |
      this.bitboards[Pieces.R] |
      this.bitboards[Pieces.Q] |
      this.bitboards[Pieces.K];
    this.occupancies[SideToMove.Black] =
      this.bitboards[Pieces.p] |
      this.bitboards[Pieces.n] |
      this.bitboards[Pieces.b] |
    ...
    if (
      this.IsSquareAttacked(
        this.side === SideToMove.White
          ? this.GetLS1B(this.bitboards[Pieces.k])
          : this.GetLS1B(this.bitboards[Pieces.K]),
        this.side
      )
    ) {
  ...
  InitHashTable(hashSize = (this.hashSize * 16) / 0x100000 || 32) {
    if (hashSize > 512 || hashSize < 1) {
      ...
      console.log(
        `Unable to set hash table size to ${hashSize}. Setting to default of 32MB`
      );
    } else {
  ...
  private WriteHash(depth: number, flag: number, score: number, move: number) {
    ...
    if (this.hashTable.key[index] < 0n || this.hashTable.key[index] === 0n) {
      throw new Error(
        `Bad hash key: ${this.hashTable.key[index]} (${this.hashKey})`
      );
    }
  ...
  private ply = 0;
  private killerMoves: number[][] = Array(2)
    .fill(0)
    .map(() => Array(this.maxPly).fill(0));
  private historyMoves: number[][] = Array(64)
    .fill(0)
    .map(() => Array(64).fill(0));
  private pvLength: number[] = [];
  private pvTable: number[][] = Array(this.maxPly)
    .fill(0)
    .map(() => Array(this.maxPly).fill(0));
  ...
  private InitEvalMasks() {
    for (let rank = 0; rank < 8; rank++) {
      for (let file = 0; file < 8; file++) {
        ...
        this.isolatedMasks[square] |=
          this.SetFileRankMask(file - 1, -1) |
          this.SetFileRankMask(file + 1, -1);
        this.wPassedMasks[square] |=
          this.SetFileRankMask(file - 1, -1) |
          this.SetFileRankMask(file, -1) |
          this.SetFileRankMask(file + 1, -1);
        this.bPassedMasks[square] |=
          this.SetFileRankMask(file - 1, -1) |
          this.SetFileRankMask(file, -1) |
          this.SetFileRankMask(file + 1, -1);
  ...
  private Evaluate() {
      ...
      if (bitboard) {
        ...
        while (bitboard) {
          ...
          if (piece <= 5) {
            ...
            mgScore -=
              this.pieceSquareValues[GamePhase.Opening][piece - 6][square ^ 56];
            egScore -=
              this.pieceSquareValues[GamePhase.Endgame][piece - 6][square ^ 56];
          }
          ...
          if (piece === Pieces.P) {
            ...
            // isolated pawns
            if (
              (this.bitboards[Pieces.P] & this.isolatedMasks[square]) ===
              0n
            ) {
          ...
          if (piece === Pieces.p) {
            ...
            // isolated pawns
            if (
              (this.bitboards[Pieces.p] & this.isolatedMasks[square ^ 56]) ===
              0n
            ) {
              mgScore += this.isolatedPenalty;
              egScore += this.isolatedPenalty;
          ...
          if (piece === Pieces.R) {
            ...
            // open file bonus
            if (
              ((this.bitboards[Pieces.P] | this.bitboards[Pieces.p]) &
                this.fileMasks[square]) ===
              0n
            ) {
          ...
          if (piece === Pieces.r) {
            // semi-open file bonus
            if (
              (this.bitboards[Pieces.p] & this.fileMasks[square ^ 56]) ===
              0n
            ) {
              mgScore -= this.fileSemiOpenScore;
              egScore -= this.fileSemiOpenScore;
    ...
    return (
      (((mgScore * (256 - phase) + egScore * phase) / 256) | 0) *
      (-1) ** this.side
    );
  }
  ...
  Search(depth: number) {
    ...
    this.scorePv = false;
    this.nodesCount = 0;
    this.killerMoves = Array(2)
      .fill(0)
      .map(() => Array(this.maxPly).fill(0));
    this.historyMoves = Array(64)
      .fill(0)
      .map(() => Array(64).fill(0));
    this.pvLength = [];
    this.pvTable = Array(this.maxPly)
      .fill(0)
      .map(() => Array(this.maxPly).fill(0));
    ...
    for (
      let currentDepth = 1;
      !this.shouldStop && currentDepth <= depth;
      currentDepth++
    ) {
      this.followPv = true;
      ...
      const pvnodes = () => {
        ...
        for (let i = 0; i < this.pvLength[0]; i++) {
          string += `${SquareToCoords[this.GetMoveSource(this.pvTable[0][i])]}${
            SquareToCoords[this.GetMoveTarget(this.pvTable[0][i])]
          }${
            this.GetMovePromoted(this.pvTable[0][i])
              ? this.promotedPieces[this.GetMovePromoted(this.pvTable[0][i])]
              : ''
          }`;
          string += ' ';
        }
      ...
      console.log(
        `info score ${infoScore()} depth ${currentDepth} nodes ${
          this.nodesCount
        } time ${Date.now() - this.timing.startTime} pv ${pvnodes()}`
      );
    ...
    console.log(
      `bestmove ${SquareToCoords[this.GetMoveSource(this.pvTable[0][0])]}${
        SquareToCoords[this.GetMoveTarget(this.pvTable[0][0])]
      }${
        this.GetMovePromoted(this.pvTable[0][0])
          ? this.promotedPieces[this.GetMovePromoted(this.pvTable[0][0])]
          : ''
      }`
    );

    return `${SquareToCoords[this.GetMoveSource(this.pvTable[0][0])]}${
      SquareToCoords[this.GetMoveTarget(this.pvTable[0][0])]
    }${
      this.GetMovePromoted(this.pvTable[0][0])
        ? this.promotedPieces[this.GetMovePromoted(this.pvTable[0][0])]
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      if (score > alpha) {
        ...
        if (!this.GetMoveCapture(move)) {
          this.historyMoves[this.GetMoveSource(move)][
            this.GetMoveTarget(move)
          ] += depth * depth;
        }
        ...
        for (
          let nextPly = this.ply + 1;
          nextPly < this.pvLength[this.ply + 1];
          nextPly++
        ) {
  ...
  private ScoreMove(move: number) {
    ...
    if (this.GetMoveCapture(move)) {
      ...
      return (
        Math.abs(this.pieceValue[this.gamePhase][Math.abs(targetPiece)]) -
        (this.GetMovePiece(move) + 1) +
        10000
      );
    } else {
      if (this.killerMoves[0][this.ply] === move) {
        ...
        return this.historyMoves[this.GetMoveSource(move)][
          this.GetMoveTarget(move)
        ];
      }
    }
  ...
  ParseUCIMove(move: string) {
    ...
    for (let i = 0; i < moves.length; i++) {
      if (
        SquareToCoords[this.GetMoveSource(moves[i])] +
          SquareToCoords[this.GetMoveTarget(moves[i])] +
          (this.GetMovePromoted(moves[i])
            ? this.promotedPieces[this.GetMovePromoted(moves[i])]
            : '') ===
  ...
  ParseUCIGo(command: string) {
    ...
    if (!movetime) {
      let movesToGo =
        parseInt((command.match(/movestogo (\d+)/) || [])[1]) || 35;
      let time, inc;
